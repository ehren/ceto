...
Ceto is an experimental language transpiling to C++ where calls may take indented blocks as arguments for a Python-inspired free-form but infix-friendly syntax extendable by ast macros. "Print should be a function", so should def, class, struct, for, while, if, defmacro, ... ("Python" with two parentheses moved or inserted). The semantics should feel a bit like Swift (classes are reference counted; structs are value types). Class instances are const propagate_const shared_ptr to const by default with ```.``` performing a safe maybe dereference for smart pointers or std::optional even in generic code. Further safety features include bounds checking for suitable containers, safe usages of raw C++ references relegated to function parameters with aliasing/invalidation prevention static_asserts added at the call site, and for-loops that revert to indexing with (references to) contiguous owning containers when a raw C++ range-based-for is not provably safe to emit.   
