...
auto contains(const T1& container,  const typename std::remove_reference_t<decltype(container)> :: value_type &  element) -> auto {
    ...
    inline auto creates_new_variable_scope(const std::shared_ptr<const Node>&  e) -> void {
        ; // pass
    }
...
struct Scope : ceto::shared_object {
    ...
    std::remove_cvref_t<decltype(false)> in_decltype = false;
        ...
        inline auto add_variable_definition(const std::shared_ptr<const Identifier>&  defined_node, const std::shared_ptr<const Node>&  defining_node) const -> void {
            ; // pass
        }
