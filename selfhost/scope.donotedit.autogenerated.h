...
struct FunctionDefinition : public ceto::shared_object, public std::enable_shared_from_this<FunctionDefinition> {
    ...
    std::shared_ptr<const Node> def_node;

    std::shared_ptr<const Identifier> function_name;
    ...
    explicit FunctionDefinition(std::shared_ptr<const Node> def_node, std::shared_ptr<const Identifier> function_name) : def_node(std::move(def_node)), function_name(std::move(function_name)) {}
...
struct Scope : public ceto::shared_object, public std::enable_shared_from_this<Scope> {
    ...
    std::vector<std::shared_ptr<const FunctionDefinition>> function_definitions = std::vector<std::shared_ptr<const FunctionDefinition>>{}; static_assert(ceto::is_non_aggregate_init_and_if_convertible_then_non_narrowing_v<decltype(std::vector<std::shared_ptr<const FunctionDefinition>>{}), std::remove_cvref_t<decltype(function_definitions)>>);
    ...
    decltype(false) is_unsafe = false;
        ...
        inline auto add_function_definition(const std::shared_ptr<const FunctionDefinition>&  function_definition) -> void {
