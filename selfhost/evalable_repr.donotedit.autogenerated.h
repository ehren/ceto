...
auto generate_loc(const T1& node) -> void {
        ...
        inline auto visit(const Node&  node) -> void override {
            ...
            auto && args { (*ceto::mad(node)).args } ;
                        [&](auto&& ceto__private__rng19){
                static_assert(requires { std::begin(ceto__private__rng19) + 2; }, "not a contiguous container");
                size_t ceto__private__size21 = std::size(ceto__private__rng19);
                for (size_t ceto__private__idx20 = 0; ; ceto__private__idx20++) {
                    if (std::size(ceto__private__rng19) != ceto__private__size21) {
                    ...
                    const auto& arg = ceto__private__rng19[ceto__private__idx20];
        ...
        inline auto visit(const BinOp&  node) -> void override {
            (this -> repr) += ((((*ceto::mad(node)).classname() + "(\"") + (*ceto::mad(node)).op) + "\", [");
            auto && args { (*ceto::mad(node)).args } ;
                        [&](auto&& ceto__private__rng22){
                static_assert(requires { std::begin(ceto__private__rng22) + 2; }, "not a contiguous container");
                size_t ceto__private__size24 = std::size(ceto__private__rng22);
                for (size_t ceto__private__idx23 = 0; ; ceto__private__idx23++) {
                    if (std::size(ceto__private__rng22) != ceto__private__size24) {
                        std::cerr << "Container size changed during iteration: " << __FILE__ << " line: "<< __LINE__ << "\n";
                        std::terminate();
                    }
                    ...
                    const auto& arg = ceto__private__rng22[ceto__private__idx23];
                                    (*ceto::mad(arg)).accept((*this));
                            (this -> repr) += ", ";
