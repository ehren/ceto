...
struct EvalableAstReprVisitor : public BaseVisitor<EvalableAstReprVisitor> {
    ...
    decltype(std::string {""}) repr = std::string {""};
        ...
        template <typename ceto__private__T1120>
auto generate_loc(const ceto__private__T1120& node) -> void {
 ...
 + ", ");
        ...
        inline auto visit(const BinOp&  node) -> void override {
            ...
            auto && args { (*ceto::mad(node)).args } ;
                ...
                size_t ceto__private__size122 = std::size(args);
                for (size_t ceto__private__idx121 = 0; ; ceto__private__idx121++) {
                    if (std::size(args) != ceto__private__size122) {
                        std::cerr << "Container size changed during iteration: " << __FILE__ << " line: "<< __LINE__ << "\n";
                        std::terminate();
                    }
                    if (ceto__private__idx121 >= ceto__private__size122) {
                        break;
                    }
                    const auto arg = args[ceto__private__idx121];
                                    (*ceto::mad(arg)).accept((*this));
                            (this -> repr) += ", ";
