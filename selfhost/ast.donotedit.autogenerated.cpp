...
using namespace pybind11::literals;
        ...
        ceto::mado(ceto::mado(ceto::mado(ceto::mado(ceto::mado(ceto::mado(ceto::mado(ceto::mado(ceto::mado(ceto::mado(ceto::mado(ceto::mado(ceto::mado(ceto::mado(ceto::mado(ceto::mado(py::class_<Scope,std::shared_ptr<Scope>>(m, "Scope"))->def(py::init<>()))->def_readwrite("indent", (&Scope::indent)))->def_readwrite("in_function_body", (&Scope::in_function_body)))->def_readwrite("in_function_param_list", (&Scope::in_function_param_list)))->def_readwrite("in_class_body", (&Scope::in_class_body)))->def_readwrite("in_decltype", (&Scope::in_decltype)))->def("indent_str", (&Scope::indent_str)))->def("add_variable_definition", (&Scope::add_variable_definition), py::kw_only(), py::arg("defined_node"), py::arg("defining_node")))->def("add_interface_method", (&Scope::add_interface_method)))->def("add_class_definition", (&Scope::add_class_definition)))->def("lookup_class", (&Scope::lookup_class)))->def("find_defs", (&Scope::find_defs), py::arg("var_node"), py::arg("find_all") = true))->def("find_def", (&Scope::find_def)))->def("enter_scope", (&Scope::enter_scope)))->def_property_readonly("parent", (&Scope::parent)))->def_property_readonly("interfaces", (&Scope::interfaces));
