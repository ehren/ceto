...
;
        const auto args = [&]() {
                auto ceto__private__ident__1 { std::vector<std::remove_cvref_t<decltype(std::string(std::declval<std::ranges::range_value_t<std::remove_cvref_t<decltype(std::span(argv, std::declval<int>()))>>>()))>>() } ;
                auto && ceto__private__ident__2 { std::span(/* unsafe: */ argv, argc) } ;
                ceto::util::maybe_reserve(ceto__private__ident__1, /* unsafe: */ ceto__private__ident__2);
                ceto::safe_for_loop<!std::is_reference_v<decltype(/* unsafe: */ ceto__private__ident__2)> && ceto::OwningContainer<std::remove_cvref_t<decltype(/* unsafe: */ ceto__private__ident__2)>>>(/* unsafe: */ ceto__private__ident__2, [&](auto &&ceto__private__lambda_param4) -> ceto::LoopControl {
    const auto a = ceto__private__lambda_param4;
