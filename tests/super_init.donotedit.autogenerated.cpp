...
using Concrete::Concrete;
    ...
    auto main() -> int {
        ...
        const auto f2 = std::make_shared<const decltype(Concrete{5})>(5);
        const auto f3 = std::make_shared<const decltype(Generic2{std::vector {{5, 6}}, std::map<int,std::string>{{1, "one"}}})>(std::vector {{5, 6}}, std::map<int,std::string>{{1, "one"}});
        const auto f4 = std::make_shared<const decltype(Concrete2{42})>(42);
        ((((std::cout << (*ceto::mad(f)).x) << (*ceto::mad(f2)).x) << ceto::bounds_check((*ceto::mad(f3)).x, 0)) << (*ceto::mad((*ceto::mad(f3)).y)).at(1)) << (*ceto::mad(f4)).x;
    }
