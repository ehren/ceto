...
struct Node : public ceto::shared_object, public std::enable_shared_from_this<Node> {
    ...
    std::vector<ceto::nonullpropconst<std::shared_ptr<const Node>>> args;
         ...
         virtual inline auto repr() const -> std::string {
            auto r { (std::string {"generic node with func "} + ((this -> func) ?
                [&]() {
 ...
 + "(" + std::to_string([&]() -> decltype(auto) { static_assert((((!std::is_reference_v<decltype((*ceto::mad(this -> args)).size())> ) && true)  || true )); return (*ceto::mad(this -> args)).size(); }()) + " args.)\n") } ;
    ...
    explicit Node(std::optional<ceto::nonullpropconst<std::shared_ptr<const Node>>> func, std::vector<ceto::nonullpropconst<std::shared_ptr<const Node>>> args) : func(func), args(std::move(args)) {}
...
struct Identifier : public Node {
    ...
    explicit Identifier(const std::string&  name) : Node (CETO_NONE, std::vector<ceto::nonullpropconst<std::shared_ptr<const Node>>>{}), name(name) {
    }
...
;
        const auto id = ceto::make_shared_nonullpropconst<const Identifier>("a");
        std::cout << (*ceto::mad(id)).name;
        const ceto::nonullpropconst<std::shared_ptr<const Node>> id_node = ceto::make_shared_nonullpropconst<const Identifier>("a"); static_assert(ceto::is_non_aggregate_init_and_if_convertible_then_non_narrowing_v<decltype(ceto::make_shared_nonullpropconst<const Identifier>("a")), std::remove_cvref_t<decltype(id_node)>>);
        std::cout << (*ceto::mad(static_pointer_cast<std::type_identity_t<ceto::nonullpropconst<std::shared_ptr<const Identifier>>> :: element_type>(ceto::get_underlying(id_node)))).name;
        std::cout << (*ceto::mad(ceto::asinstance<const Identifier>(id_node))).name;
        ...
        const auto args2 = std::vector<ceto::nonullpropconst<std::shared_ptr<const Node>>>{id, id_node};
        static_cast<void>(args2);
        const auto node = ceto::make_shared_nonullpropconst<const Node>(id, args);
        std::cout << ((*ceto::mad(node)).func == CETO_NONE);
        std::cout << "\n" << (*ceto::mad(node)).repr();
