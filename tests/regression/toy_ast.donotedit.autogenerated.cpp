...
) + "(") + std::to_string((*ceto::mad(this -> args)).size())) + " args.)\n") } ;
...
struct Identifier : public Node {
    ...
    auto main() -> int {
        ...
        std::cout << (*ceto::mad(static_pointer_cast<std::type_identity_t<std::shared_ptr<const Identifier>> :: element_type>(id_node))).name;
        ...
        std::cout << (ceto::maybe_bounds_check_access((*ceto::mad(node)).args,0) == nullptr);
        (std::cout << "\n") << (*ceto::mad(node)).repr();
        std::cout << (*ceto::mad(ceto::maybe_bounds_check_access((*ceto::mad(node)).args,0))).repr();
    }
