...
template <typename _ceto_private_C4, typename _ceto_private_C5>struct Bar : ceto::shared_object {
    ...
    explicit Bar(_ceto_private_C4 a, _ceto_private_C5 b, std::shared_ptr<const Foo<decltype(a),decltype(b),decltype(b)>> f) : a(a), b(b), f(f) {}
...
template <typename _ceto_private_C6, typename _ceto_private_C7>struct Bar2 : ceto::shared_object {
    ...
    explicit Bar2(_ceto_private_C6 a, _ceto_private_C7 b, decltype(std::make_shared<const decltype(Foo{b, b, b})>(b, b, b)) f) : a(a), b(b), f(f) {}
...
template <typename _ceto_private_C8>struct MixedGenericConcrete : ceto::shared_object {
    ...
    explicit MixedGenericConcrete(_ceto_private_C8 a, int b) : a(a), b(b) {}
    ...
    auto main() -> int {
        ...
        (std::cout << ceto::mado(ceto::mado(ceto::mado(ceto::mado(f5)->c)->b)->c)->c) << std::string {"\n"};
        const auto b = std::make_shared<const decltype(Bar{1, 2, f})>(1, 2, f);
        (((((std::cout << ceto::mado(b)->a) << ceto::mado(b)->b) << ceto::mado(ceto::mado(b)->f)->a) << ceto::mado(ceto::mado(b)->f)->b) << ceto::mado(ceto::mado(b)->f)->c) << std::string {"\n"};
        ...
        (((((std::cout << ceto::mado(b2)->a) << ceto::mado(b2)->b) << ceto::mado(ceto::mado(b2)->f)->a) << ceto::mado(ceto::mado(b2)->f)->b) << ceto::mado(ceto::mado(b2)->f)->c) << std::string {"\n"};
