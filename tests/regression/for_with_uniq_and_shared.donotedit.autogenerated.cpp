
#include <string>
#include <cstdio>
#include <cstdlib>
#include <iostream>
#include <fstream>
#include <sstream>
#include <functional>
#include <cassert>
#include <compare> // for <=>
#include <thread>
#include <optional>


#include "ceto.h"


;

;

;

;

;

;

;

;

;

;

;
#include "ceto_private_listcomp.donotedit.autogenerated.h"
;
#include "ceto_private_boundscheck.donotedit.autogenerated.h"
;
#include "ceto_private_convenience.donotedit.autogenerated.h"
;
#include "ceto_private_append_to_pushback.donotedit.autogenerated.h"
;
struct Uniq : public ceto::object {

    decltype(0) x = 0;

        inline auto bar() -> auto {
            (this -> x) = ((this -> x) + 1);
            printf("in bar %d %p\n", this -> x, [&]() -> decltype(auto) { static_assert(((!std::is_reference_v<decltype(static_cast<const void *>(this))>  || (!std::is_reference_v<decltype("in bar %d %p\n")> && std::is_fundamental_v<std::remove_cvref_t<decltype("in bar %d %p\n")>> && !std::is_reference_v<decltype(this -> x)> && std::is_fundamental_v<std::remove_cvref_t<decltype(this -> x)>>)) && true)); return static_cast<const void *>(this); }());
            return (this -> x);
            (this -> x) = ((this -> x) + 1);
            printf("in bar %d %p\n", this -> x, [&]() -> decltype(auto) { static_assert(((!std::is_reference_v<decltype(static_cast<const void *>(this))>  || (!std::is_reference_v<decltype("in bar %d %p\n")> && std::is_fundamental_v<std::remove_cvref_t<decltype("in bar %d %p\n")>> && !std::is_reference_v<decltype(this -> x)> && std::is_fundamental_v<std::remove_cvref_t<decltype(this -> x)>>)) && true)); return static_cast<const void *>(this); }());
            return (this -> x);
        }

};

struct Shared : public ceto::shared_object, public std::enable_shared_from_this<Shared> {

    decltype(0) x = 0;

        inline auto foo() const -> auto {
            printf("foo\n");
            return 10;
        }

};

template <typename ceto__private__C1>struct Shared2 : public ceto::enable_shared_from_this_base_for_templates {

    ceto__private__C1 x;

    explicit Shared2(ceto__private__C1 x) : x(std::move(x)) {}

    Shared2() = delete;

};

    auto main() -> int {
        const auto x = 5;
        
            auto&& ceto__private__intermediate2 = std::vector {{1, 2, 3}};

            static_assert(requires { std::begin(ceto__private__intermediate2) + 2; }, "not a contiguous container");
            size_t ceto__private__size4 = std::size(ceto__private__intermediate2);
            for (size_t ceto__private__idx3 = 0; ; ceto__private__idx3++) {
                if (std::size(ceto__private__intermediate2) != ceto__private__size4) {
                    std::cerr << "Container size changed during iteration: " << __FILE__ << " line: "<< __LINE__ << "\n";
                    std::terminate();
                }
                if (ceto__private__idx3 >= ceto__private__size4) {
                    break;
                }
                 auto &&  x = ceto__private__intermediate2[ceto__private__idx3];
                            printf("%d\n", [&]() -> decltype(auto) { static_assert(((!std::is_reference_v<decltype(x)>  || (!std::is_reference_v<decltype("%d\n")> && std::is_fundamental_v<std::remove_cvref_t<decltype("%d\n")>>)) && true)); return x; }());
                    x = (x + 1);

            }
            static_assert(std::is_const_v<decltype(x)>);
        auto lst { std::vector {{1, 2, 3}} } ;
        
    
            static_assert(requires { std::begin(lst) + 2; }, "not a contiguous container");
            size_t ceto__private__size6 = std::size(lst);
            for (size_t ceto__private__idx5 = 0; ; ceto__private__idx5++) {
                if (std::size(lst) != ceto__private__size6) {
                    std::cerr << "Container size changed during iteration: " << __FILE__ << " line: "<< __LINE__ << "\n";
                    std::terminate();
                }
                if (ceto__private__idx5 >= ceto__private__size6) {
                    break;
                }
                 auto &&  x = lst[ceto__private__idx5];
                            printf("%d\n", [&]() -> decltype(auto) { static_assert(((!std::is_reference_v<decltype(x)>  || (!std::is_reference_v<decltype("%d\n")> && std::is_fundamental_v<std::remove_cvref_t<decltype("%d\n")>>)) && true)); return x; }());
                    x = (x + 1);

            }
            
    
            static_assert(requires { std::begin(lst) + 2; }, "not a contiguous container");
            size_t ceto__private__size8 = std::size(lst);
            for (size_t ceto__private__idx7 = 0; ; ceto__private__idx7++) {
                if (std::size(lst) != ceto__private__size8) {
                    std::cerr << "Container size changed during iteration: " << __FILE__ << " line: "<< __LINE__ << "\n";
                    std::terminate();
                }
                if (ceto__private__idx7 >= ceto__private__size8) {
                    break;
                }
                 auto &&  x = lst[ceto__private__idx7];
                            printf("%d\n", [&]() -> decltype(auto) { static_assert(((!std::is_reference_v<decltype(x)>  || (!std::is_reference_v<decltype("%d\n")> && std::is_fundamental_v<std::remove_cvref_t<decltype("%d\n")>>)) && true)); return x; }());
                    x = (x + 1);

            }
            auto u { std::vector<ceto::propagate_const<std::unique_ptr<decltype(Uniq())>>>() } ;
        auto s { std::vector<ceto::propagate_const<std::shared_ptr<const decltype(Shared())>>>() } ;
        auto s2 { std::vector<ceto::propagate_const<std::shared_ptr<const decltype(Shared2{std::string {"blah"}})>>>() } ;
        
            auto&& ceto__private__intermediate9 = std::vector {{1, 2, 3, 4, 5}};

            static_assert(requires { std::begin(ceto__private__intermediate9) + 2; }, "not a contiguous container");
            size_t ceto__private__size11 = std::size(ceto__private__intermediate9);
            for (size_t ceto__private__idx10 = 0; ; ceto__private__idx10++) {
                if (std::size(ceto__private__intermediate9) != ceto__private__size11) {
                    std::cerr << "Container size changed during iteration: " << __FILE__ << " line: "<< __LINE__ << "\n";
                    std::terminate();
                }
                if (ceto__private__idx10 >= ceto__private__size11) {
                    break;
                }
                const auto x = ceto__private__intermediate9[ceto__private__idx10];
                            (u).push_back(ceto::make_unique_propagate_const<Uniq>());
                    (s).push_back(ceto::make_shared_propagate_const<const Shared>());
                    (s2).push_back(ceto::make_shared_propagate_const<const decltype(Shared2{std::string {"blah"}})>(std::string {"blah"}));

            }
            
    
            static_assert(requires { std::begin(u) + 2; }, "not a contiguous container");
            size_t ceto__private__size13 = std::size(u);
            for (size_t ceto__private__idx12 = 0; ; ceto__private__idx12++) {
                if (std::size(u) != ceto__private__size13) {
                    std::cerr << "Container size changed during iteration: " << __FILE__ << " line: "<< __LINE__ << "\n";
                    std::terminate();
                }
                if (ceto__private__idx12 >= ceto__private__size13) {
                    break;
                }
                const auto x = u[ceto__private__idx12];
                            printf("%d\n", [&]() -> decltype(auto) { static_assert(((!std::is_reference_v<decltype(x -> bar())>  || (!std::is_reference_v<decltype("%d\n")> && std::is_fundamental_v<std::remove_cvref_t<decltype("%d\n")>>)) && true)); return x -> bar(); }());
                    printf("%d\n", [&]() -> decltype(auto) { static_assert(((!std::is_reference_v<decltype((*ceto::mad(x)).bar())>  || (!std::is_reference_v<decltype("%d\n")> && std::is_fundamental_v<std::remove_cvref_t<decltype("%d\n")>>)) && true)); return (*ceto::mad(x)).bar(); }());

            }
            int n { 0 } ; static_assert(std::is_convertible_v<decltype(0), decltype(n)>);
        
    
            static_assert(requires { std::begin(u) + 2; }, "not a contiguous container");
            size_t ceto__private__size15 = std::size(u);
            for (size_t ceto__private__idx14 = 0; ; ceto__private__idx14++) {
                if (std::size(u) != ceto__private__size15) {
                    std::cerr << "Container size changed during iteration: " << __FILE__ << " line: "<< __LINE__ << "\n";
                    std::terminate();
                }
                if (ceto__private__idx14 >= ceto__private__size15) {
                    break;
                }
                const auto x = u[ceto__private__idx14];
                            printf("bar again: %d\n", [&]() -> decltype(auto) { static_assert(((!std::is_reference_v<decltype((*ceto::mad(x)).bar())>  || (!std::is_reference_v<decltype("bar again: %d\n")> && std::is_fundamental_v<std::remove_cvref_t<decltype("bar again: %d\n")>>)) && true)); return (*ceto::mad(x)).bar(); }());
                    n = (n + 1);
                    if ((n % 2) == 0) {
                        (*ceto::mad(x)).bar();
                    }

            }
            
    
            static_assert(requires { std::begin(u) + 2; }, "not a contiguous container");
            size_t ceto__private__size17 = std::size(u);
            for (size_t ceto__private__idx16 = 0; ; ceto__private__idx16++) {
                if (std::size(u) != ceto__private__size17) {
                    std::cerr << "Container size changed during iteration: " << __FILE__ << " line: "<< __LINE__ << "\n";
                    std::terminate();
                }
                if (ceto__private__idx16 >= ceto__private__size17) {
                    break;
                }
                const auto x = u[ceto__private__idx16];
                            printf("bar again again: %d\n", [&]() -> decltype(auto) { static_assert(((!std::is_reference_v<decltype((*ceto::mad(x)).bar())>  || (!std::is_reference_v<decltype("bar again again: %d\n")> && std::is_fundamental_v<std::remove_cvref_t<decltype("bar again again: %d\n")>>)) && true)); return (*ceto::mad(x)).bar(); }());

            }
            auto v { std::vector {ceto::make_shared_propagate_const<Shared>()} } ;
        auto v2 { std::vector {ceto::make_shared_propagate_const<const Shared>()} } ;
        
    
            static_assert(requires { std::begin(s) + 2; }, "not a contiguous container");
            size_t ceto__private__size19 = std::size(s);
            for (size_t ceto__private__idx18 = 0; ; ceto__private__idx18++) {
                if (std::size(s) != ceto__private__size19) {
                    std::cerr << "Container size changed during iteration: " << __FILE__ << " line: "<< __LINE__ << "\n";
                    std::terminate();
                }
                if (ceto__private__idx18 >= ceto__private__size19) {
                    break;
                }
                const auto i = s[ceto__private__idx18];
                            (*ceto::mad(i)).foo();
                    (v2).push_back([&]() -> decltype(auto) { static_assert(((!std::is_reference_v<decltype(i)> ) && true || ceto::IsContainer<std::remove_cvref_t<decltype(v2)>>)); return i; }());

            }
            
    
            static_assert(requires { std::begin(v2) + 2; }, "not a contiguous container");
            size_t ceto__private__size21 = std::size(v2);
            for (size_t ceto__private__idx20 = 0; ; ceto__private__idx20++) {
                if (std::size(v2) != ceto__private__size21) {
                    std::cerr << "Container size changed during iteration: " << __FILE__ << " line: "<< __LINE__ << "\n";
                    std::terminate();
                }
                if (ceto__private__idx20 >= ceto__private__size21) {
                    break;
                }
                const auto vv2 = v2[ceto__private__idx20];
                            (*ceto::mad(vv2)).foo();

            }
            const auto s1 = ceto::make_shared_propagate_const<Shared>();
        
    
            static_assert(requires { std::begin(v) + 2; }, "not a contiguous container");
            size_t ceto__private__size23 = std::size(v);
            for (size_t ceto__private__idx22 = 0; ; ceto__private__idx22++) {
                if (std::size(v) != ceto__private__size23) {
                    std::cerr << "Container size changed during iteration: " << __FILE__ << " line: "<< __LINE__ << "\n";
                    std::terminate();
                }
                if (ceto__private__idx22 >= ceto__private__size23) {
                    break;
                }
                const auto v1 = v[ceto__private__idx22];
                            (*ceto::mad(v1)).x = 55;
                    (std::cout << "v:") << (*ceto::mad(v1)).foo();

            }
        }

