...
auto moretest2(const ceto__private__T11& p) -> void {
    ...
    inline auto foo(const std::vector<std::string>&  items) -> void {
        std::cout << "size: " << (*ceto::mad(items)).size() << "\n";
            ...
            static_assert(ceto::ContiguousContainer<std::remove_cvref_t<decltype(items)>>, "this loop requires a contiguous container (e.g. std.vector is contiguous, std.map is not)");
