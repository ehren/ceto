...
auto bad7(const ceto__private__T114& f,  auto &  v2) -> void {
            ...
            static_assert(requires { std::begin(ceto__private__intermediate15) + 2; }, "not a contiguous container");
...
struct HoldsFunc : public ceto::shared_object, public std::enable_shared_from_this<HoldsFunc> {
    ...
    auto main() -> int {
        ...
        ok_lambda([&]() -> decltype(auto) { static_assert(((!std::is_reference_v<decltype(ceto::bounds_check((*ceto::mad(f)).vec, 0))> ) && ceto::IsStateless<std::remove_cvref_t<decltype(ok_lambda)>>)); return ceto::bounds_check((*ceto::mad(f)).vec, 0); }());
        const auto ok_lambda2 = [f = ceto::default_capture(f)](const auto &x) {
                return mutates(f);
                };
        ok_lambda2([&]() -> decltype(auto) { static_assert(((!std::is_reference_v<decltype(ceto::bounds_check((*ceto::mad(f)).vec, 0))> ) && ceto::IsStateless<std::remove_cvref_t<decltype(ok_lambda2)>>)); return ceto::bounds_check((*ceto::mad(f)).vec, 0); }());
        ...
        ok_func_member_copy([&]() -> decltype(auto) { static_assert(((!std::is_reference_v<decltype(ceto::bounds_check((*ceto::mad(f)).vec, 0))> ) && ceto::IsStateless<std::remove_cvref_t<decltype(ok_func_member_copy)>>)); return ceto::bounds_check((*ceto::mad(f)).vec, 0); }());
