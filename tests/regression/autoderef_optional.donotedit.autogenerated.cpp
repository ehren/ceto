...
#include "ceto.h"
    ...
    auto main() -> int {
        const std::optional<std::string> x = std::string {"blah"}; static_assert(ceto::is_non_aggregate_init_and_if_convertible_then_non_narrowing_v<decltype(std::string {"blah"}), std::remove_cvref_t<decltype(x)>>);
        std::optional<std::string> xm = std::string {"blah"}; static_assert(ceto::is_non_aggregate_init_and_if_convertible_then_non_narrowing_v<decltype(std::string {"blah"}), std::remove_cvref_t<decltype(xm)>>);
