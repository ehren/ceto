...
;
    auto main() -> int {
        const std::unordered_map<decltype(std::string {"one"}),decltype(1)> m = [&]() {
                ...
                if constexpr (!std::is_void_v<decltype(std::unordered_map<decltype(std::string {"one"}),decltype(1)>{{std::string {"one"}, 1}, {std::string {"two"}, 2}})>) { return std::unordered_map<decltype(std::string {"one"}),decltype(1)>{{std::string {"one"}, 1}, {std::string {"two"}, 2}}; } else { static_cast<void>(std::unordered_map<decltype(std::string {"one"}),decltype(1)>{{std::string {"one"}, 1}, {std::string {"two"}, 2}}); };
        ...
        std::map<decltype(1),decltype(2)> mm = [&]() {
                ...
                if constexpr (!std::is_void_v<decltype(std::map<decltype(1),decltype(2)>{{1, 2}, {2, 3}})>) { return std::map<decltype(1),decltype(2)>{{1, 2}, {2, 3}}; } else { static_cast<void>(std::map<decltype(1),decltype(2)>{{1, 2}, {2, 3}}); };
                }(); static_assert(ceto::is_non_aggregate_init_and_if_convertible_then_non_narrowing_v<decltype([&]() {
                static_assert(true && std::is_same_v<std::map<decltype(2),decltype(3)>,std::map<decltype(1),decltype(2)>>, "all key-value pairs must be of the same type in map literal");
                if constexpr (!std::is_void_v<decltype(std::map<decltype(1),decltype(2)>{{1, 2}, {2, 3}})>) { return std::map<decltype(1),decltype(2)>{{1, 2}, {2, 3}}; } else { static_cast<void>(std::map<decltype(1),decltype(2)>{{1, 2}, {2, 3}}); };
