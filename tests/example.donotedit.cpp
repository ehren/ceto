...
#include <future>
;
template <typename ceto__private__C22>struct Foo : public ceto::enable_shared_from_this_base_for_templates {

    ceto__private__C22 data_member;

        template <typename ceto__private__T123>
auto method(const ceto__private__T123& param) const -> auto {
    ...
    explicit Foo(ceto__private__C22 data_member) : data_member(std::move(data_member)) {}

    Foo() = delete;

};

    template <typename ceto__private__T124>
auto calls_method(const ceto__private__T124& f) -> auto {
        return (*ceto::mad(f)).method(f);
    }

struct UniqueFoo : public ceto::object {

    std::vector<ceto::nonullpropconst<std::unique_ptr<const UniqueFoo>>> consumed = std::vector<ceto::nonullpropconst<std::unique_ptr<const UniqueFoo>>>{}; static_assert(ceto::is_non_aggregate_init_and_if_convertible_then_non_narrowing_v<decltype(std::vector<ceto::nonullpropconst<std::unique_ptr<const UniqueFoo>>>{}), std::remove_cvref_t<decltype(consumed)>>);

        inline auto size() const -> auto {
            return (*ceto::mad(this -> consumed)).size();
        }

        inline auto consuming_method( ceto::nonullpropconst<std::unique_ptr<const UniqueFoo>>  u) -> void {
            (*ceto::mad(ceto::make_shared_nonullpropconst<const decltype(Foo{42})>(42))).method(u);
...
;
        const auto args = []( auto &&  ceto__private__ident__2) {
                ...
                ceto::util::maybe_reserve(ceto__private__ident__1, ceto__private__ident__2);
                    ...
                    size_t ceto__private__size26 = std::size(ceto__private__ident__2);
                    for (size_t ceto__private__idx25 = 0; ; ceto__private__idx25++) {
                        if (std::size(ceto__private__ident__2) != ceto__private__size26) {
                        ...
                        const auto a = ceto__private__ident__2[ceto__private__idx25];
        ...
        ceto::util::maybe_reserve(ceto__private__ident__1, ceto__private__ident__2);
            ...
            size_t ceto__private__size28 = std::size(ceto__private__ident__2);
            for (size_t ceto__private__idx27 = 0; ; ceto__private__idx27++) {
                if (std::size(ceto__private__ident__2) != ceto__private__size28) {
                    std::cerr << "Container size changed during iteration: " << __FILE__ << " line: "<< __LINE__ << "\n";
                    std::terminate();
                }
                ...
                const auto a = ceto__private__ident__2[ceto__private__idx27];
                            (ceto__private__ident__1).push_back(std::string([&]() -> decltype(auto) { static_assert((((!std::is_reference_v<decltype(a)> ) && true)  || true || ceto::IsContainer<std::remove_cvref_t<decltype(ceto__private__ident__1)>>)); return a; }()));
        ...
        auto u2 = ceto::make_unique_nonullpropconst<const UniqueFoo>();
