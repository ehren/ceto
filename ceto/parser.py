...
def parse_include(module: Identifier) -> typing.Tuple[str, Module]:
    ...
    repr_path = os.path.join(dirname, module_name + ".donotedit.danger_passed_to_python_eval.cth.cetorepr")
    ...
    if os.path.isfile(repr_path):
        ...
        if repr_time > module_time:
            with open(repr_path) as f:
                repr_file = f.read()
            return cpp_module_path, expand_includes(eval(repr_file))

    with open(module_path) as f:
        source = f.read()

    parsed_module = parse_no_include_expand(source)

    with open(repr_path, "w") as f:
        f.write(parsed_module.ast_repr())

    return cpp_module_path, expand_includes(parsed_module)
...
def parse_no_include_expand(source: str):
...
def parse(source: str):
    p = parse_no_include_expand(source)
