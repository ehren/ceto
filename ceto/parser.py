...
def parse_include(module: Identifier) -> typing.Tuple[str, Module]:
    ...
    repr_path = os.path.join(dirname, module_name + ".donotedit.danger_passed_to_python_eval.cetorepr")

    if os.path.isfile(repr_path):
        module_time = os.path.getmtime(module_path)
        repr_time = os.path.getmtime(module_path)
        if repr_time > module_time:
            with open(repr_path) as f:
                repr_file = f.read()
            return cpp_module_path, eval(repr_file)

    with open(module_path) as f:
        source = f.read()

    parsed_module = parse(source)

    # with open(repr_path, "w") as f:
    #     f.write(parsed_module.ast_repr())

    return cpp_module_path, parsed_module
...
def parse(source: str):
        ...
        for call in res.args:
            if isinstance(call, Call) and call.func.name == "include":
                ...
                if module.name not in seen_modules:
                    module_path, module_ast = parse_include(module)
    ...
    # t = perf_counter()
    # evalable_repr = res.ast_repr()
    # print(f"repr time {perf_counter() - t}")
    # print("repr", evalable_repr)
    # t = perf_counter()
    # print("eval", eval(evalable_repr))
    # print(f"eval time {perf_counter() - t}")
