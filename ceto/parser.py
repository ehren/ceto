...
def parse_included_module(module_name: str) -> typing.Tuple[str, Module]:
...
def _handle_module(module: str):
    # print("handle mod", module)
    module_path, module_ast = parse_included_module(module)
    # print("done handle mod", module, module_ast)
    return module, module_path, module_ast#.ast_repr()
...
def expand_includes(node: Module):
    ...
    with concurrent.futures.ProcessPoolExecutor() as executor:
        for call in node.args:
            if isinstance(call, Call) and call.func.name == "include":
                ...
                if module.name not in seen_modules:
                    ...
                    print("submitting", module.name)
                    # futures.append(executor.submit(parse_included_module, module.name))
                    include_calls[module.name] = call
                    futures.append(executor.submit(_handle_module, module.name))
        ...
        for future in concurrent.futures.as_completed(futures):
            module_name, module_path, module_ast_repr = future.result()
            # module_name, module_path, module_ast = future.result()
            print("dont modeee", module_name)
            # module_path, module_ast = future.result()
            module_ast = eval(module_ast_repr)
            module_ast = expand_includes(module_ast)
            modules[module_name] = module_path, module_ast
        ...
        #             if module_name.name in modules and module_name.name not in expanded_modules:# or True:
        #                 expanded_modules.add(module_name.name)
        #                 assert isinstance(module_name, Identifier)
        #                 module_path, module_ast = modules[module_name.name]
        #                 del modules[module_name.name]
        #                 call.args = [module_name]
        #                 index = node.args.index(call)
        #                 for a in module_ast.args:
        #                     set_file_path(a, module_path)
        #                 new_args = node.args[0:index] + module_ast.args + node.args[index:]
        ...
        for module_name, call in include_calls.items():
            assert isinstance(call, Call) and call.func.name == "include"
            if module_name in modules and module_name not in expanded_modules:# or True:
                expanded_modules.add(module_name)
                module_path, module_ast = modules[module_name]
                del modules[module_name]
                call.args = [Identifier(module_name)]
                index = node.args.index(call)
                for a in module_ast.args:
                    set_file_path(a, module_path)
                node.args = node.args[0:index] + module_ast.args + node.args[index:]
                # break
...
def parse_from_cmdargs(cmdargs):
    ...
    return expand_includes(eval(_parse_maybe_cached(filename, repr_path)))
